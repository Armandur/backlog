...
// Anstrangning är den hårdast belastade kvoten agenten har, som andel. Ett
// fönster vars nollställning passerat räknas inte: kvoten har börjat om, och
// siffran säger ingenting om läget nu.
func (a Anvandning) Anstrangning() (float64, bool) {
	hogst, hittad := 0.0, false
	nu := timeutil.Now()
	for _, fonster := range []*Kvotfonster{a.FemTimmar, a.SjuDagar} {
		if fonster == nil || fonster.NollstallsAt <= nu {
			continue
		}
		if !hittad || fonster.Andel > hogst {
			hogst, hittad = fonster.Andel, true
		}
	}
	return hogst, hittad
}
